...
export class MenuListComponent implements OnInit, AfterViewInit {
  ...
  elements: any = [];
  headElements = ['#', 'image','name', 'description', 'DOR', 'status', ''];
 ...
 updateStatus(id){
  this.GlobalService.updateStatus('menu/item/statusUpdate', id).subscribe((res)=>{
    console.log(res)
    if(res['success']){
 ...
 delete(id){
  console.log(id);
  this.GlobalService.delete('menu/item', id).subscribe((res)=>{
    console.log(res)
    if(res['success']){
      this.baseApp.showSuccess(res['success_subject'], res['success_message']);
      this.getItemList();
